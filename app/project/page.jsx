...
async function ProjectList() {
  ...
  return (
    ...
    <div className='flex gap-2 my-2 mx-2'>
      <Link href={"/project/addproject"} className='bg-green-600 font-bold text-white py-3 px-6 rounded'>Add Project</Link>
    </div>
        ...
        return (
          <div className="flex p-4 flex-grow md:flex-grow-0 border border-slate-300 rounded-lg my-3 justify-between w-[48%] gap-5 items-start" key={project._id}>
            ...
            <div className="project-actions">
              ...
              <Link
                className="edit"
                href={`/project/edit-project/${project._id}`}
                title="Edit"
              >
                <HiPencilAlt size={32} className='text-blue-600'/>
              </Link>
              <Link href={`/project/${project._id}`} title="View " >
